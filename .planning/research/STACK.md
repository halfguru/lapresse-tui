...
**Confidence:** HIGH (versions verified via crates.io API, Context7 docs, and official GitHub repos)
...
**Critical compatibility note:** ratatui-image 10.0.6 depends on `ratatui ^0.30.0` and `image ^0.25.6`. These versions are locked together — do not upgrade one without verifying the other.
...
**Why refinery over rusqlite_migration:** refinery is more widely used, supports embedded migrations via `include_str!`, and has better docs for complex migration chains. rusqlite_migration is simpler but less flexible for evolving schemas.
...
# Images
ratatui-image = { version = "10.0", default-features = true, features = ["crossterm"] }
...
**Lock these together (do not upgrade independently):**
...
**When upgrading ratatui:**
1. Check ratatui-image's Cargo.toml for the matching ratatui version
2. Check ratatui's changelog for crossterm version requirements
3. Upgrade all three together in one commit

## Key Architecture Implications

1. **Thread model:** tokio multi-thread runtime for scraping. TUI runs on main thread with crossterm events. Use `tokio::sync::mpsc` channels to send scraped data from background tasks to the UI.

2. **Image pipeline:** Download image bytes via reqwest → decode with `image::ImageReader` → store as `DynamicImage` → pass to `ratatui_image::picker::Picker::new_resize_protocol()` → render via `StatefulImage` widget.

3. **SQLite access:** rusqlite is synchronous. Run DB operations on a dedicated tokio task using `tokio::task::spawn_blocking` to avoid blocking the async runtime. Consider a `tokio::sync::oneshot` channel pattern for request-response DB queries.

4. **Scraper rate limiting:** La Presse will block aggressive scraping. Implement configurable delay between requests (start with 2-5 seconds). Use tokio timers, not thread sleep.
