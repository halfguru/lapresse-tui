...
impl App {
                                ...
                                .with_guessed_format()
                            {
                                Ok(rdr) => {
                                    if let Ok(dyn_img) = rdr.decode() {
                                        ...
                                        let alt_text = reader.article.images.get(idx)
                                            .and_then(|img| img.alt_text.clone());
                                        reader.images[idx] = ImageLoadState::Loaded(Box::new(ImageState {
                                            protocol,
                                            alt_text,
                                        }));
                                    } else {
    ...
    fn load_images(&mut self, full: &FullArticle) -> (Vec<ImageLoadState>, Option<Receiver<ImageLoadMsg>>) {
